...
def to_relative_path(path: str, root: str):
    try:
        # remove prefix: ./xxx/xxx/.obj or xxx/xxx/.obj
        parts = re.split(r'/|\\', path)
        if len(parts) > 4 and parts[0] == '.' and parts[3] == '.obj':
            path = '/'.join(parts[4:])
            return normpath(path)
        if len(parts) > 3 and parts[2] == '.obj':
            path = '/'.join(parts[3:])
            return normpath(path)
        # remove toolchain dir prefix
        if root == None or root.strip() == '':
            if TOOLCHAIN_ROOTDIR != None and basename(TOOLCHAIN_ROOTDIR) in path:
                result = normpath(relpath(path, TOOLCHAIN_ROOTDIR))
                if not isabs(result):
                    result = join('[lib]', 'misc', result)
                return result
            else:
                return normpath(path)
        else:
            return normpath(relpath(path, root))
    except Exception as e:
        return normpath(path)

class _Parser(with_metaclass(ABCMeta, object)):
    """Internal interface for parsing"""
    SECTIONS = ('.text', '.data', '.bss', '.heap', '.stack', '.rodata')
    ...
    def parse_mapfile(self, file_desc):
        ...
        for name, stats in self.modules.items():
            if name.startswith("[lib]"):
                ...
                new_modules[to_relative_path(name, common_prefix)] = stats
            else:
                new_modules[name] = stats
        return new_modules
...
class _ArmccParser(_Parser):
    ...
    RE_OBJECT = re.compile(r'(.+\.(l|lib|ar))\((.+\.o(bj)?)\)')
    OBJECT_EXTENSIONS = (".o", ".obj")
...
class _IarParser(_Parser):
    ...
    def parse_section(self, line):
        ...
        test_re = re.match(self.RE, line)
        if test_re:
            if test_re.group(2) == 'ro code':
                section = '.text'
            elif test_re.group(2) == 'const':
                section = '.rodata'
    ...
    def parse_command_line(self, lines):
        ...
        self.cmd_modules = {s: to_relative_path(f, common_prefix)
...
class MemapParser(object):
    ...
    print_sections = ('.text', '.data', '.bss', '.rodata')
    delta_sections = ('.text-delta', '.data-delta', '.bss-delta', '.rodata-delta')
    ...
    ROM_FORMAT_STR = (
        "Total Flash memory (text + data + rodata): {}({:+}) bytes\n"
    )
    ...
    def compute_report(self):
        ...
        self.mem_summary = {
            ...
            'total_flash': (self.subtotal['.text'] + self.subtotal['.data'] + self.subtotal['.rodata']),
            'total_flash_delta':
            self.subtotal['.text-delta'] + self.subtotal['.data-delta'] + self.subtotal['.rodata-delta'],
        }
...
def main():
    ...
    parser = ArgumentParser(
        description="Memory Map File Analyser. \nVersion: %s" %
        version)
    ...
    if args.file and args.toolchain:
        mapViewPath = args.file + '.view'
        if exists(mapViewPath):
            global TOOLCHAIN_BINDIR, TOOLCHAIN_ROOTDIR
            with open(mapViewPath, 'r', encoding='utf-8') as f:
                mapViewData = yaml.safe_load(f)
                if 'compilerPath' in mapViewData:
